...
{#if form?.berhasil}
	<div class="kabar ok">
		...
		· gas {form.gas?.toLocaleString('id-ID')} · biaya {rupiah(form.biayaWei)}
	</div>
{:else if form?.pesan}
...
{#if b.status === 'menunggu_persetujuan'}
	<section class="kartu sorot">
		...
		{#if bolehTerbit}
			{#if !konfirmasi}
				<button class="utama" onclick={() => (konfirmasi = true)}>Setujui &amp; terbitkan</button>
			{:else}
				<div class="konfirmasi">
					...
					<form method="POST" action="?/terbitkan"
						...
						<button type="submit" class="utama" disabled={sedangKirim}>
							{sedangKirim ? 'Mengirim transaksi…' : `Ya, terbitkan ${b.jumlah.toLocaleString('id-ID')} sertifikat`}
						</button>
						<button type="button" class="sekunder" onclick={() => (konfirmasi = false)}>Batal</button>
					</form>
				</div>
			{/if}
		{:else}
...
{#if b.status === 'gagal'}
	...
	{#if bolehTerbit}
		<form method="POST" action="?/terbitkan" use:enhance>
			<button type="submit" class="utama">Coba terbitkan lagi</button>
		</form>
	{/if}
{/if}
...
<section class="kartu">
	...
	<dl>
		...
		{#if b.txHash}
			...
			<dt>Biaya</dt><dd>{rupiah(b.biayaWei)} · {(Number(b.biayaWei) / b.jumlah / 1e18 * 50_000_000).toFixed(4)} per sertifikat</dd>
		{/if}
...
<style>
	...
	dl { display: grid; grid-template-columns: minmax(130px, 175px) 1fr; gap: 7px 18px; margin: 0; font-size: 13px; }
	dl.ringkas { margin-bottom: 16px; }
	dt { color: var(--redup); }
	dd { margin: 0; min-width: 0; }

	.pil { font-size: 11.5px; padding: 3px 11px; border-radius: 999px; white-space: nowrap; }
	...
	button { border: 0; border-radius: 8px; padding: 10px 17px; font-size: 13.5px; font-weight: 550; cursor: pointer; }
	.utama { background: var(--aksen); color: #fff; }
	.utama:disabled { opacity: 0.55; cursor: default; }
	.sekunder { background: transparent; color: var(--tx); border: 1px solid var(--garis); margin-left: 8px; }
	...
	table { width: 100%; border-collapse: collapse; font-size: 12.5px; min-width: 430px; }
